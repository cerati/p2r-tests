...
#define nthreads 1
 ...
 constexpr size_t PosInMtrx(size_t i, size_t j, size_t D) {
  return i*D+j;
}
...
public:
        ...
        ) const -> void
    {
       ...
       for (size_t ti=BlockIdx; ti< nb*nevts; ti+=BlockExtent){
          int ie = ti/nb;
          int ib = ti%nb;
          int range = nb*nevts;
          printf("Accessing  ti = %i, ie=%i, ib=%i,  Total block in stream = %i \n",int(ti),  ie, ib, range);
        ...
        //if((ti)%100==0 ) printf("index = %i ,(blockIdx,blockExt)=(%i,%i), ,(ThreadIdx,ThreadExt)=(%i,%i), track = (%.3f) \n ",
...
int main (int argc, char* argv[]) {
   ...
   std::cout<<"input trk"<&((*trk)[0]).par.data[8]<<std::endl;
   ...
   //Idx const numElements(nMPTRK*bsize);
