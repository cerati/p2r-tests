...
struct MPNX {
   ...
   ALPAKA_FN_ACC  inline void load(MPNX<T, N, 1>& dst, const int b) const {
#pragma unroll
   ...
   ALPAKA_FN_ACC  inline void save(const MPNX<T, N, 1>& src, const int b) {
#pragma unroll
...
struct MPTRK {
  ...
  ALPAKA_FN_ACC  inline const auto load_component (const int batch_id) const{//b is a batch idx
  ...
  ALPAKA_FN_ACC  inline void save_component(MPTRK_ &src, const int batch_id) {
...
struct MPHIT {
  ...
  MP3x3SF cov;
  //
  ALPAKA_FN_ACC  inline const auto load_component(const int batch_id) const {
    MPHIT_ dst;
...
ALPAKA_FN_ACC inline void MultHelixProp(const MP6x6F_ &a, const MP6x6SF_ &b, MP6x6F_ &c) {//ok
...
ALPAKA_FN_ACC inline void MultHelixPropTransp(const MP6x6F_ &a, const MP6x6F_ &b, MP6x6SF_ &c) {//
...
ALPAKA_FN_ACC inline float hipo(const float x, const float y) {return std::sqrt(x*x + y*y);}

ALPAKA_FN_ACC inline void KalmanUpdate(MP6x6SF_ &trkErr_, MP6F_ &inPar_, const MP3x3SF_ &hitErr_, const MP3F_ &msP_){	  
...
ALPAKA_FN_ACC inline void propagateToR(const MP6x6SF_ &inErr_, const MP6F_ &inPar_, const MP1I_ &inChg_, 
