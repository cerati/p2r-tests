...
struct MP1I {
  int data[1*bsize+PAD(1,bsize)];
};

struct MP22I {
  int data[22*bsize+PAD(22,bsize)];
};

struct MP1F {
  float data[1*bsize+PAD(1,bsize)];
};

struct MP2F {
  float data[2*bsize+PAD(2,bsize)];
};

struct MP3F {
  float data[3*bsize+PAD(3,bsize)];
};

struct MP6F {
  float data[6*bsize+PAD(6,bsize)];
};

struct MP3x3 {
  float data[9*bsize+PAD(9,bsize)];
};
struct MP3x6 {
  float data[18*bsize+PAD(18,bsize)];
};
...
struct MP6x6SF {
  float data[21*bsize+PAD(21,bsize)];
};

struct MP6x6F {
  float data[36*bsize+PAD(36,bsize)];
};
...
void propagateToR(const MP6x6SF* inErr, const MP6F* inPar, const MP1I* inChg, 
                  const MP3F* msP, MP6x6SF* outErr, MP6F* outPar, struct MP6x6F* errorProp, 
		  struct MP6x6F* temp) {
...
#pragma acc parallel loop gang worker collapse(2)                                   \
      default(present) num_workers(NUM_WORKERS) vector_length(VECTOR_LENGTH)        \
      private(errorProp, temp, rotT00, rotT01, resErr_loc, kGain, res_loc, newErr)
     for (size_t ie=0;ie<nevts;++ie) { // loop over events
       for (size_t ib=0;ib<nb;++ib) { // loop over bunches of tracks
          ...
          for(size_t layer=0; layer<nlayer; ++layer) {
            ...
            propagateToR(&(*btracks).cov, &(*btracks).par, &(*btracks).q, &(*bhits).pos, &(*obtracks).cov, &(*obtracks).par,&errorProp,&temp); // vectorized function
