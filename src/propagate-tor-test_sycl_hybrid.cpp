...
clang++ -std=c++20 -O2 -fsycl --gcc-toolchain={path_to_gcc} src/propagate-toz-test_sycl_hybrid.cpp -o test-sycl.exe -Dntrks=8192 -Dnevts=100 -DNITER=5 -Dbsize=1 -Dnlayer=20
...
decltype(auto) p2r_get_queue(){
  if constexpr ( enable_gpu_backend == false) {
    std::cout << "WARNING: running regular std::par version on CPU.\n" << std::endl;
  } else {
    std::cout << "WARNING: running regular std::par version on GPU.\n" << std::endl;
  }
  return 0;
}
...
requires SYCLCompute<is_sycl_target>
decltype(auto) p2r_get_allocator(queue_t& cq){
...
//SYLC specialized version:
template <typename queue_tp, bool is_sycl_target>
requires SYCLCompute<is_sycl_target>
void dispatch_p2r_kernel(auto&& p2r_kernel, queue_tp& cq, const int outer_loop_range){
  //
  try {
     cq.submit([&](sycl::handler &h){
         h.parallel_for(sycl::range(outer_loop_range), p2r_kernel);
       });
...
int main (int argc, char* argv[]) {
   ...
   //
   auto cq = p2r_get_queue<is_sycl_backend, enable_gpu_backend>();
   //
   auto MPTRKAllocator = p2r_get_allocator<MPTRK, decltype(cq), is_sycl_backend>(cq);
   auto MPHITAllocator = p2r_get_allocator<MPHIT, decltype(cq), is_sycl_backend>(cq);
   //
...
#pragma unroll                         
   ...
   dispatch_p2r_kernel<decltype(cq), is_sycl_backend>(p2r_kernels, cq, outer_loop_range);
